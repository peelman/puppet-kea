...
#     kea::dhcp4::subnets:
#       '192.168.1.0/24':
#         name: 'office-lan'    # Results in office-lan.json instead of 192.168.1.0-24.json
#         pools: [...]
#
#   Without a name key, filenames are derived from the CIDR:
#     192.168.1.0/24 -> 192.168.1.0-24.json
#
...
class kea::dhcp4::subnets {
  ...
  $shared_networks_dir = "${kea::config_dir}/shared-networks4.d"
  $shared_networks_file = "${kea::config_dir}/kea-dhcp4-shared-networks.json"
  ...
  $shared_networks = lookup('kea::dhcp4::shared_networks', Hash, 'deep', {})
  ...
  $subnet_names.each |String $name| {
    ...
    # User-provided name:  'office-lan' -> 'office-lan.json'
    # Derived from subnet: '192.168.1.0/24' -> '192.168.1.0-24.json'
    $user_name = $config['name']
    if $user_name {
      # Sanitize user-provided name, collapse repeated underscores
      $safe_name_raw = regsubst($user_name, '[^a-zA-Z0-9._-]', '_', 'G')
      $safe_name = regsubst($safe_name_raw, '_+', '_', 'G')
    } else {
      # Use CIDR with slash replaced by dash: 192.168.1.0/24 -> 192.168.1.0-24
  ...
  $shared_network_names.each |String $sn_name| {
    ...
    $sn_subnets = pick($sn_config['subnets'], []).map |$idx, Hash $subnet| {
      ...
      $auto_id = fqdn_rand(2147483646, "kea-sn-${sn_name}-subnet-${idx}") + 1
      ...
      # lint:ignore:140chars
      $base + $pools + $option_data + $reservations + $relay + $interface + $valid_lifetime + $renew_timer + $rebind_timer + $extra
      # lint:endignore
    }
    ...
    # lint:ignore:140chars
    $sn_full_config = $sn_base + $sn_interface + $sn_option_data + $sn_relay + $sn_valid_lifetime + $sn_renew_timer + $sn_rebind_timer + $sn_extra + { 'subnet4' => $sn_subnets }
    # lint:endignore
